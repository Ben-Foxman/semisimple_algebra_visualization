"""Generate a JPG illustrating two Brauer factorization rules."""
...
def row_one_lines() -> list[str]:
    ...
    left_top = ["propa", "focusa", "propb", "propd", "propc", "propb"]
    left_bottom = ["focusa", "propc", "propd", "focusb", "propa", "focusb"]
    right_top = ["propa", "propc", "propb", "propb", "focusa", "propd"]
    right_bottom = ["propa", "propc", "focusb", "focusb", "focusa", "propd"]

    lines.extend(
        row_annotation(
            0,
            r"{\scalebox{1.28}{$\textbf{Brauer 1}$}}",
            r"$\begin{aligned}\pi_1&=(2\,5)(4\,6)\\[2pt]\pi_2&=(1'\,5')(3'\,6')\end{aligned}$",
        )
    )
...
def row_two_lines() -> list[str]:
    lines: list[str] = []
    y_shift = ROW_TOPS[1]
    centered_left_shift = LEFT_SHIFT + COL_STEP / 2
    centered_right_shift = RIGHT_SHIFT + COL_STEP / 2

    left_top = ["propb", "focusa", "focusb", "focusb", "propb"]
    left_bottom = ["propc", "propc", "propa", "focusa", "propa"]
    right_top = ["propb", "propb", "focusb", "focusb", "focusa"]
    right_bottom = ["propc", "propc", "propa", "propa", "focusa"]

    lines.extend(
        row_annotation(
            1,
            r"{\scalebox{1.28}{$\textbf{Brauer 2}$}}",
            r"$\begin{aligned}\pi_1&=(2\,5)\\[2pt]\pi_2&=(4'\,5')\end{aligned}$",
        )
    )
    lines.extend(draw_diagram(centered_left_shift, y_shift, left_top, left_bottom, r"$D_a\in B_{5}(d)$"))
    lines.extend(draw_diagram(centered_right_shift, y_shift, right_top, right_bottom))
    lines.extend(
        brace_lines(
            centered_right_shift,
            y_shift,
            1,
            4,
            r"$D_b\in B_{4}(d)$",
...
\usepackage[paperwidth=43cm,paperheight=28cm,margin=0pt]{{geometry}}
\usepackage{{amsmath}}
\usepackage{{graphicx}}
\usepackage{{tikz}}
...
\definecolor{{prope}}{{RGB}}{{116, 203, 232}}
\definecolor{{propf}}{{RGB}}{{255, 197, 92}}
\definecolor{{focusa}}{{RGB}}{{255, 98, 57}}
\definecolor{{focusb}}{{RGB}}{{247, 206, 63}}
\definecolor{{focusc}}{{RGB}}{{240, 73, 98}}
\definecolor{{cupa}}{{RGB}}{{255, 166, 120}}
\definecolor{{capb}}{{RGB}}{{255, 193, 216}}
\pagestyle{{empty}}
...
def main() -> int:
    output = (
        ...
        if len(sys.argv) > 1
        else Path(__file__).with_name("brauer_factorization_rules.jpg")
    )
    output.parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory() as temp_dir:
        workdir = Path(temp_dir)
        tex_path = workdir / "brauer_factorization_rules.tex"
